...
def sort_scores(scores: list[int], direction: Literal["asc", "desc"]) -> list[int]:
...
def day_number(day: Weekday) -> int:
    """Every Weekday value is handled below — no catch-all `case _`
    needed, mypy can see the match is already exhaustive over the Literal.
