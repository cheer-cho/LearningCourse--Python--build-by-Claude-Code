# Reference solution for checkpoint_03 — see checkpoint_03.py at the
# module root for the scenario.
...
def play_round(secret: int, guesses) -> str:
    ...
    for attempt, guess in enumerate(guesses, start=1):
        feedback = guess_feedback(secret, guess, attempt, max_attempts)
        line = f"attempt {attempt}: guess {guess} -> {feedback}"
        transcript = line if not transcript else transcript + "\n" + line
        if feedback == "correct!":
            break
    else:
        if not transcript:
            transcript = "no guesses made"
    return transcript
